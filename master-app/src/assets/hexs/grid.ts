export class GridClass {
    ...
    hexagon(q: number, r: number, radius: number, solid: boolean): any {
        let currentRing = null, i = null, ref = null, result = null;
        result = [];
        if (solid) {
          result.push({
            q: q,
    ...
    ring(q: number, r: number, radius: number): any {
        let i, j, len, moveDirection, moveDirectionIndex, moveDirections, ref, result;
        result = [];
  ...
  getCenterXY(q: number, r: number): any {
    var x, y;
    if (this.pointyTiles) {
